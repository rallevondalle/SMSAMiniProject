function [cleanData] = myDenoiser(x,fs, denoiseLength, denoiseThreshold)

%Generate signal data and noise
fs     = fs;
f      = x;
denoiseLength = denoiseLength;                           % 1 second
denoiseThreshold = denoiseThreshold;

dt     = 1/fs;                              % time resolution, 'sample rate'
t      = 0:dt:denoiseLength;                % time vector
fclean = f; %sin(2*pi*50*t) + sin(2*pi*120*t);  % sum of 2 frequencies
...
denoiseThreshold = denoiseThreshold;    % denoise threshold
